...
install_oci8() {
  ...
  mkdir -p $1/.profile.d
  echo "export LD_LIBRARY_PATH=\"\${LD_LIBRARY_PATH}:${oracle_instant_client_dir}\"" > $1/.profile.d/nodejs.sh
  echo "export LD_LIBRARY_PATH=\"\${LD_LIBRARY_PATH}:${oracle_instant_client_dir}\"" >> $2/export
